...
def landing_page():
    ...
    try:
        result = DB.selectOne("SELECT artists FROM IS601_Top_Artists WHERE curMonth = %s and curYear= %s", curMonth, curYear)
        output = json.loads(str(result.row["artists"]))
    except Exception as e:
        e = "An error occurred when retrieving records. Try again."
        flash(str(e), "danger")
    
    if not result.row:
        top_artists_url = "https://spotify81.p.rapidapi.com/top_20_by_monthly_listeners"
        headers = {
            "X-RapidAPI-Key": current_app.api_key,
            "X-RapidAPI-Host": current_app.api_host
        }
        response = requests.request("GET", top_artists_url, headers=headers)
        output = response.json()
        try:
            result = DB.insertOne("INSERT INTO IS601_Top_Artists(artists, curMonth, curYear) VALUES(%s,%s,%s) ON DUPLICATE KEY UPDATE artists=%s, curMonth=%s, curYear=%s"
                                    , json.dumps(output), curMonth, curYear, json.dumps(output), curMonth, curYear)
        except Exception as e:
            e = "An error occurred when retrieving records. Try again."
            flash(str(e), "danger")
    return render_template("landing_page.html", args=output)
